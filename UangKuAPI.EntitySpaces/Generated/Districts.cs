...
Date Generated       : 9/28/2024 4:20:36 PM
===============================================================================
*/
...
namespace UangKuAPI.EntitySpaces.Generated
{
	...
	abstract public partial class esDistricts : esEntity
	{
		...
		public virtual bool LoadByPrimaryKey(string disID)
		{
			if(this.es.Connection.SqlAccessType == esSqlAccessType.DynamicSQL)
				return LoadByPrimaryKeyDynamic(disID);
			else
				return LoadByPrimaryKeyStoredProcedure(disID);
		}

		public virtual bool LoadByPrimaryKey(esSqlAccessType sqlAccessType, string disID)
		{
		...
		private bool LoadByPrimaryKeyDynamic(string disID)
		{
			DistrictsQuery query = new DistrictsQuery("Districts");
			query.Where(query.DisID == disID);
			return Load(query);
		}

		private bool LoadByPrimaryKeyStoredProcedure(string disID)
		{
			esParameters parms = new esParameters();
			parms.Add("DisID", disID);
		...
		public DistrictsQuery Query
		{
			get
			{
				if (query == null)
				{
                    query = new DistrictsQuery("Districts");
					InitQuery(query);
				}
		...
		{
            InitQuery((DistrictsQuery)query);
		}
	...
	public partial class DistrictsMetadata : esMetadata, IMetadata
	{
		...
		protected DistrictsMetadata()
		{
			...
			c = new esColumnMetadata(ColumnNames.DisID, 0, typeof(int), esSystemType.Int32);
			c.PropertyName = PropertyNames.DisID;
			c.IsAutoIncrement = true;
			c.NumericPrecision = 11;
			m_columns.Add(c);
				
			c = new esColumnMetadata(ColumnNames.DisName, 1, typeof(string), esSystemType.String);
			c.PropertyName = PropertyNames.DisName;
			c.CharacterMaxLength = 255;
			c.IsNullable = true;
			m_columns.Add(c);
				
			c = new esColumnMetadata(ColumnNames.CityID, 2, typeof(int), esSystemType.Int32);
